...
export function handleMint(event: MintEvent): void {
  ...
  // The total equity deposit fields are running totals of the equity paid for the balance of the position, without any pnl
  // considered. They are updated on mints, redeems, and transfers (not price changes or rebalances).
  position.totalEquityDepositInCollateral = position.totalEquityDepositInCollateral.plus(equityAddedInCollateral)
  position.totalEquityDepositInDebt = position.totalEquityDepositInDebt.plus(equityAddedInDebt)
  position.save()
...
export function handleRedeem(event: RedeemEvent): void {
  ...
  const equityDepositedForSharesInCollateral = event.params.actionData.shares.times(position.totalEquityDepositInCollateral).div(position.balance)
  const equityDepositedForSharesInDebt = event.params.actionData.shares.times(position.totalEquityDepositInDebt).div(position.balance)
  ...
  pnl.realized = equityRemovedInCollateral.minus(equityDepositedForSharesInCollateral)
  pnl.equityReceived = equityRemovedInCollateral
  ...
  position.totalEquityDepositInCollateral = position.totalEquityDepositInCollateral.minus(equityDepositedForSharesInCollateral)
  position.totalEquityDepositInDebt = position.totalEquityDepositInDebt.minus(equityDepositedForSharesInDebt)
  position.realizedPnl = position.realizedPnl.plus(pnl.realized)
  position.save()
